...
// 로그인 시, jwt 토큰 발급 함수
func IssueAccessJWT(userID int64) (string, error) {
	if userID <= 0 {
...
func IssueRefreshJWT(userID int64) (string, error) {
	if userID <= 0 {
		return "", errors.New("user id is empty")
	}

	now := time.Now().In(env.Seoul)
	ExpiredTime := now.Add(time.Hour * time.Duration(env.Config.Auth.RefreshTokenExpire))
	claims := types.JWTClaims{
		userID,
		jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(ExpiredTime),
